...
import { MessageCircleQuestion, Clock, Plus, X, Send, Loader2, ChevronDown, Lock } from 'lucide-react';
...
const CreateRoomModal = ({ onClose, onSuccess }) => {
  const [form, setForm] = useState({ title: '', scheduledAt: '', price: '' });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await api.post('/qa', { title: form.title, scheduledAt: form.scheduledAt || null, price: form.price ? parseFloat(form.price) : 0 });
      onSuccess();
    } catch (err) {
      ...
      <motion.div initial={{ y: 40, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 40, opacity: 0 }}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-1.5 block">Scheduled At</label>
...
const QARoomDetail = ({ room, currentUserId, onClose }) => {
  ...
  const isHost = room.hostId?._id === currentUserId || room.hostId === currentUserId || room.isHost;
  const isPaid = room.price > 0;
  const canAccess = isHost || !isPaid || hasPaid;
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      const { data } = await api.post(`/qa/${room._id}/payment/order`);
      const options = {
        key: data.keyId,
        amount: data.amount,
        ...
        handler: async (response) => {
          try {
            const { data: v } = await api.post(`/qa/${room._id}/payment/verify`, response);
            if (v.success) {
              setHasPaid(true);
              setLoading(true);
              api.get(`/qa/${room._id}/questions`).then(({ data }) => setQuestions(data)).finally(() => setLoading(false));
            }
  ...
  const handleAnswer = async (questionId) => {
    ...
    } catch (err) { alert(err?.response?.data?.error || 'Failed to answer'); }
  };
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {isPaid && (
                <span className="flex items-center gap-1 px-2 py-0.5 bg-primary/20 text-primary text-[10px] font-bold rounded-full">
                  {hasPaid || isHost ? '✓ Access Granted' : `₹${room.price}`}
                </span>
              )}
            </div>
        ...
        {!canAccess ? (
          <div className="flex-1 flex flex-col items-center justify-center p-8 gap-4">
            ...
            <div className="text-center">
              <p className="font-black text-lg mb-1">Paid Session</p>
              <p className="text-sm text-muted-foreground">Pay ₹{room.price} to access this Q&A session and ask questions</p>
            </div>
            <button onClick={handlePay} disabled={paying}
              className="px-8 py-3 bg-primary text-primary-foreground rounded-2xl font-bold shadow-lg shadow-primary/20 hover:scale-[1.02] active:scale-95 transition-all disabled:opacity-50 flex items-center gap-2">
              {paying ? <Loader2 size={18} className="animate-spin" /> : null}
              {paying ? 'Processing...' : `Pay ₹${room.price} & Join`}
            </button>
          </div>
        ) : (
          <>
                        ...
                        <div className="ml-4 space-y-2">
                          ...
                          <div className="flex gap-2">
                            <button onClick={() => handleAnswer(q._id)}
                              className="flex-1 py-2 bg-primary text-primary-foreground rounded-xl text-xs font-bold hover:scale-[1.02] transition-all">Post Answer</button>
                            <button onClick={() => { setAnsweringId(null); setAnswer(''); }}
                              className="px-3 py-2 bg-accent rounded-xl text-xs font-bold">Cancel</button>
                          </div>
                        </div>
                      ) : (
                        <button onClick={() => { setAnsweringId(q._id); setAnswer(''); }}
                          className="ml-4 text-xs text-primary font-bold hover:underline">Answer this question</button>
                      )
                    ) : (
...
export default function QARooms() {
  ...
  return (
    <div className="max-w-2xl mx-auto pb-24">
      ...
      {loading ? (
        <div className="space-y-4">{[1,2,3].map(i => <div key={i} className="glass-card p-5 rounded-2xl animate-pulse h-28" />)}</div>
      ) : rooms.length === 0 ? (
          ...
          {rooms.map((room, idx) => (
            <motion.div key={room._id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.05 }}
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-1">
                    ...
                    {room.isHost && <span className="px-2 py-0.5 bg-primary/20 text-primary text-[10px] font-bold rounded-full uppercase tracking-wider">Host</span>}
                    {room.price > 0 && (
                      <span className="flex items-center gap-1 px-2 py-0.5 bg-amber-500/20 text-amber-400 text-[10px] font-bold rounded-full">
                        <Lock size={10} /> ₹{room.price}
                      </span>
                    )}
                  </div>
                  <p className="text-xs text-muted-foreground">
                    by <span className="font-bold text-foreground">{room.hostName || 'Unknown'}</span>
                    {room.skillName && <span className="ml-2 text-primary font-bold">· {room.skillName}</span>}
                  </p>
              ...
              <div className="mt-4 flex items-center justify-between">
                <p className="text-xs text-muted-foreground">{room.price > 0 ? `₹${room.price} to join` : 'Free session'} · Click to open</p>
      ...
      <AnimatePresence>
        {showCreate && <CreateRoomModal onClose={() => setShowCreate(false)} onSuccess={() => { setShowCreate(false); fetchRooms(); }} />}
        {activeRoom && <QARoomDetail room={activeRoom} currentUserId={currentUserId} onClose={() => setActiveRoom(null)} />}
      </AnimatePresence>
    </div>
  );
}
