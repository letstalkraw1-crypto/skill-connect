...
const listDocuments = async (req, res) => {
  try {
    ...
    const documents = await Document.find(query).populate('authorId', 'name').populate('skillId', 'name').sort({ updatedAt: -1 }).lean();
    res.json(documents);
  } catch (err) {
...
const createDocument = async (req, res) => {
  try {
    ...
    const document = new Document({ _id: uuidv4(), skillId, title, content, authorId: req.user.userId });
    await document.save();
...
const getDocument = async (req, res) => {
  try {
    const document = await Document.findById(req.params.id).populate('authorId', 'name').lean();
...
module.exports = { listDocuments, createDocument, getDocument, updateDocument };
