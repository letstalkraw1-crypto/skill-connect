...
router.post('/', verifyToken, async (req, res) => {
  try {
    ...
    // shortCode is now generated by the model's pre-save hook
    const newEvent = new Event({
      ...
      venueName: venueName || req.body.venue_name || null,
      venueCoords: venueCoords || req.body.venue_coords || null
    });
    ...
    await creatorRsvp.save().catch(e => console.error('Auto-RSVP failed:', e));
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Internal server error' });
  }
...
try {
  ...
  if (event.creatorId && event.creatorId.toString() === userId.toString()) {
